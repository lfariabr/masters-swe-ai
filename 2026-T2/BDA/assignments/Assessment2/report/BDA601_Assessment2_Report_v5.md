...
*BDA601 Big Data and Analytics - Assessment 2 - Visualisation and Model Development (v5)*
...
in AUC (Appendix C). This suggests limited rather than severe overfitting. Test AUC was 0.802, well
...
The tree and EDA agree: highest-risk customers are newer subscribers on month-to-month contracts,
...
customers best (AUC 0.833); at threshold 0.30 it achieved recall 0.754 and churn-F1 0.618. Weighting
changes the training objective, threshold tuning changes the decision rule, and the forest improves
ranking by combining trees. The decision tree remains the required explanatory model; the forest is
the stronger operational candidate (Appendix A). Threshold tuning leaves AUC unchanged because it
does not reorder customer risk scores; it only changes where predictions switch from stay to churn.
...
10:1 were sensitivity scenarios, not measured facts. At 5:1, validation selected a Random Forest
threshold of 0.14. Test recall reached 0.933, but precision fell to 0.418 and 486 false alarms were
generated (Figure 3). The business budget owner must validate this trade-off against incentive cost
...
sizes (Appendix D). These descriptive gaps cannot be attributed solely to the model because support
...
> Table A1 - Held-out comparison of the baseline decision tree and recall-focused alternatives.
...
> Table B1 - Random Forest sensitivity to illustrative false-negative and false-positive cost ratios.
...
## Appendix C - Generalisation diagnostic

> Table C1 - Train and test performance of the baseline decision tree at the default 0.50 threshold.
...
## Appendix D - Held-out demographic error audit

> Table D1 - Held-out decision-tree error rates across gender and senior-citizen groups.
