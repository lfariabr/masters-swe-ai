...
*BDA601 Big Data and Analytics - Assessment 1 - Report (v6, submission-ready draft)*
...
Dong, X. L., & Srivastava, D. (2015). *Big data integration*. Synthesis Lectures on Data Management, 7(1), 1-198. Morgan & Claypool. https://doi.org/10.2200/S00578ED1V01Y201404DTM040
...
| Identity resolution | Linking records that refer to the same customer across sources, despite inconsistent or missing IDs (record linkage). |
| Data fusion | Deciding the authoritative value for a field when matched sources disagree - the third integration step, after schema alignment and record linkage. |
| Enrichment | Adding context (external data) or derived attributes (e.g. customer lifetime value, product affinity) to make data more analytically useful. |
...
flowchart TD
    ...
    J --> EN["Enrich<br/>external context + derived features"]
    EN --> K["Curate Gold datasets"]
...
**Table E2. Worked example - field types and a sample record (Sales transaction DB).** *(illustrates the per-column typing the brief expects - numerical, categorical, datetime - with sample values; the same treatment applies to every source in Table E1.)*

| Field | Data type | Example value |
|---|---|---|
| order_id | Categorical (ID) | ORD-1048772 |
| customer_id | Categorical (ID) | CUST-00913 |
| guest_id | Categorical (nullable ID) | (null) |
| sku | Categorical (ID) | SKU-3391 |
| qty | Numerical (discrete) | 2 |
| price | Numerical (continuous) | 49.95 |
| discount | Numerical (continuous) | 5.00 |
| payment_status | Categorical | paid |
| order_ts | Datetime | 2026-06-14T13:42:08+09:30 |
...
The full set of integration challenges referenced in §4: schema alignment and duplicate resolution first (the two the rubric names) - which, with data fusion, form the canonical **schema alignment → record linkage → data fusion** sequence (Dong & Srivastava, 2015) - then identity, timing, quality, privacy, and external-source issues, each with a resolution step and the artefact it produces.
...
| **Conflicting field values (data fusion)** | Matched records disagree - two addresses or phones for one customer | Choose the authoritative value by source priority + recency; keep alternates with lineage | Golden customer record |
...
| Rubric attribute | Weight | Status in v6 |
|---|---:|---|
...
| V's **evaluated** (SLO a) | - | ✅ §2 + Figure 1 (five canonical V's + Valence, severity-ranked) |
...
| Body word count 1,350–1,650 | - | ✅ body is prose + Figure 1 only (zero tables); prose ~1,575 (v6: +16 words) - inside range under any reading |
...
| §5 Data lake (prose) | 300–360 | 370 |
| §6 Assumptions & conclusion | 120–180 | 171 |
| **Body total** | **≈ 1,575** | **1,575** |
...
(Version label realigned to v6; v4/v5 were PDF-only exports.) All changes align the report to Dr. Chen's Week 3 lecture guidance on Assessment 1, without altering the argument or structure:

1. **§2 V-framing** - reframed as the **five canonical V's plus Valence** (the lecturer scoped A1 to five V's; Valence kept as the value-unlock and flagged as a deliberate extension).
2. **§5.1** - named AWS as the **single** platform, **chosen over Azure** (the lecturer's exact framing); open-source column clarified as a portability reference.
3. **Appendix E - Table E2** - added a worked example showing **per-column data types + a sample record** (the lecturer asked for example rows and numerical/categorical typing).
4. **Appendix B - Figure B1** - added an explicit **Enrich** step (external context + derived features) so enrichment is named in the workflow.
5. **Appendix G** - added the **data fusion** challenge (authoritative-value selection), completing the **schema alignment → record linkage → data fusion** trio.
6. **Appendix A** - glossary entries for **Data fusion** and **Enrichment**; tied Identity resolution to record linkage.
7. **Reference added** - Dong & Srivastava (2015), cited in Appendix G for the integration trio.

Body grew ~16 words (1,559 → 1,575) - still inside the 1,350-1,650 band.
