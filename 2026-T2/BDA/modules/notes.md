...
- **SLO d)** design analytical models. **Assessment 2 is due at the end of this week (26/07/2026).** ⚠️ Activity 1's `load_boston()` was **removed in scikit-learn 1.2** on ethical grounds - its `B` column, `1000(Bk - 0.63)²`, is a **non-invertible** transform encoding the assumption that racial self-segregation raises house prices ([scikit-learn deprecation notice](https://scikit-learn.org/1.0/modules/generated/sklearn.datasets.load_boston.html)). Formula decomposed + working data-loading workaround in the module notes. Full detail: [module08_notes.md](module-08-predictive-modelling/module08_notes.md).
...
- **K-means (the module's core algorithm):** pick k random centroids → assign every point to its nearest centroid → recompute centroids as the mean of their points → repeat until convergence. **Distance-based**, so scale features (typically `StandardScaler`) whenever their magnitudes aren't already comparable (unlike Module 6's scale-invariant decision trees).
...
### TLDR
- **What:** association rules find if-then patterns in unlabelled transactional data - "if a customer buys bread, they likely also buy butter" (**market basket analysis**, R4). Unsupervised, like Module 9's clustering, but asks a different question: item-level co-occurrence within records, not record-level grouping (R5).
- **Three metrics, one job each:** **support** (how often X and Y appear together, out of everyone), **confidence** (of the X-buyers, what fraction also buy Y - direction matters, `{A⇒B} ≠ {B⇒A}`), **lift** (is Y more likely given X than Y's own baseline rate - the tie-breaker that catches confidence's "Y is just generally popular" trap) (R3, R5).
- **Algorithm family, one throughline:** AIS → SETM → Apriori → AprioriTID → AprioriHybrid all suffer from expensive candidate generation and repeated full database scans (R1). **FP-Growth** fixes both by compressing transactions into an **FP-tree** once (2 scans total) and mining it recursively with no candidate generation - a strong general-purpose choice across this module's resources (R1, R2), though **Eclat** (vertical TID-list intersection, R5) can win instead on dense data with few unique items.
- **Two famous real-world hooks:** Walmart's pre-hurricane Strawberry Pop-Tart spike, and the Friday-evening beer-and-diapers pattern (R4) - both illustrate that ARM surfaces *correlation*, and a human still supplies the causal story.
- **Bridges to Module 9:** clusters are broad, generic patterns over large groups; association rules are narrow and granular (a rule's support might be a couple of percent, not tens) - "two sides of the same coin," per McCormick's video (R3), looking at the same customer data through different lenses.
