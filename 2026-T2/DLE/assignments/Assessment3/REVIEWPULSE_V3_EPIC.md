...
**Implementation baseline:** ReviewPulse `main` at merged PR [#100](https://github.com/lfariabr/review-pulse/pull/100), commit `0f02be3`<br>
**Backlog reconciled:** 29 July 2026 against the submitted A2 report, frozen experiments and current GitHub state<br>
...
**Completed six-model track:** [#94](https://github.com/lfariabr/review-pulse/issues/94) + [#95](https://github.com/lfariabr/review-pulse/issues/95) -> [#96](https://github.com/lfariabr/review-pulse/issues/96) -> supplemental evidence in #88/#89
...
The remaining A3 work is report- and release-driven:
...
- include the completed #94-#96 GRU/CNN work as a clearly labelled supplemental six-model comparison without replacing the canonical experiment;
...
The optional GRU and CNN variants passed their gates through PRs #98-#100. The submitted four-model experiment remains canonical, while the frozen six-model output is supplemental evidence. Automatic aspect extraction, Topic Modelling and Laptops remain unactivated extensions.
...
Issue #85 closed via PR #97 on 25 July 2026. The v3 interface now presents exact-offset ATAE-LSTM attention and deterministic DistilBERT gradient x input attribution, explicitly marks review-only baselines as unsupported, exports the submitted `food`/`service` RQ3 example and limits every view to indicative rather than causal evidence.

Issues #94-#96 closed through PRs #98, #99 and #100. The merged six-model baseline records 266 passing tests and 8 expected skips, preserves the canonical four-model outputs, and publishes the supplemental comparison at artifact commit `cef08fa` and evaluation commit `941148c`.
...
| ReviewPulse v3.0 | DLE602 | Three-class sentiment for each specified aspect in a review | Core: TF-IDF, target-agnostic LSTM, ATAE-LSTM, DistilBERT; exploratory: target-agnostic GRU and review-only CNN | Changes the learning task to ABSA and investigates aspect conditioning, architecture, efficiency and indicative interpretability |
...
- the post-PR #100 baseline records 266 passing and 8 skipped tests; this suite must be rerun during packaging.
...
3. retain the post-#100 baseline and rerun the complete suite at release;
...
| [#94](https://github.com/lfariabr/review-pulse/issues/94) | Optional target-agnostic GRU variant | Luis | **Closed via PR #98** | Matched GRU artifact, metadata, adapter and tests exist |
| [#95](https://github.com/lfariabr/review-pulse/issues/95) | Optional review-only CNN baseline | Luis | **Closed via PR #99** | Text CNN artifact, metadata, adapter and tests exist |
| [#96](https://github.com/lfariabr/review-pulse/issues/96) | Optional six-model comparison integration | Luis + Group | **Closed via PR #100** | Canonical four-model and supplemental six-model paths both remain reproducible |
...
PRs #90, #92, #93, #97, #98, #99 and #100 form the merged implementation baseline. Issue #28 belongs to the older v2 artifact-hosting backlog and is not a blocker for DLE602 A3.
...
environment + legacy baseline
  ...
  -> GRU [#94] + CNN [#95] [closed]
  -> supplemental six-model integration [#96, closed]
  -> report [#88] -> ZIP + v3.0.0 [#89]
```
...
| Gated six-model extension | Completed 28 Jul | GRU, CNN and supplemental comparison | #94-#96 closed without replacing the canonical experiment |
| Report + submission package | 29 Jul-19 Aug | Report, source, instructions, artifacts and ZIP | #88 and #89 close against frozen experimental evidence |
...
- Tensor shapes and masks for both core LSTM models and the completed GRU/CNN edge cases.
...
- supplemental six-model table from the frozen #96 output, labelled exploratory.
- confusion matrices.
...
| Remaining v3 changes regress the merged legacy/v3 test baseline | Medium | High | Preserve APIs and artifact paths; retain the 266-pass post-#100 baseline and rerun at the release gate | Revert the affected seam before packaging proceeds |
...
- [x] #94 and #95 independently satisfy their model, training, artifact, inference and test contracts.
- [x] #96 preserves the canonical four-model outputs and produces a separately labelled supplemental six-model comparison.
- [x] GRU and CNN remain review-only and explicitly report token evidence as unsupported.
- [x] Optional results, including negative results, are traceable to the same data/provenance rules and are labelled exploratory for #88/#89.
