...
**Core critical path:** [#85](https://github.com/lfariabr/review-pulse/issues/85) -> [#88](https://github.com/lfariabr/review-pulse/issues/88) -> [#89](https://github.com/lfariabr/review-pulse/issues/89)<br>
...
| Evaluation + interface | 9-13 Aug | Four-model comparison, error analysis, evidence views and Streamlit v3 | #84 complete; implement #85 |
...
1. Implement #85: aligned ATAE heatmap, aligned DistilBERT evidence and explicit unsupported-model states.
2. Hold the extension activation gate. If #85 is stable and the two-day budget remains, implement #94 and #95 in parallel as independent branches.
3. Merge #94/#95 only after their isolated acceptance checks, then execute #96 to preserve the canonical four-model path and add the supplemental six-model path.
4. Freeze core and any accepted optional artifacts, then write #88 with four-model A2 results plus clearly labelled exploratory evidence.
5. Complete #89 only after clean-install CPU smoke, artifact/ZIP size checks and final source-to-report consistency.
