...
**Implementation baseline:** ReviewPulse `main` at merged PR [#97](https://github.com/lfariabr/review-pulse/pull/97), commit `8f9f2a3`<br>
**Backlog reconciled:** 25 July 2026 against the submitted A2 report and current GitHub state<br>
**Core critical path:** [#88](https://github.com/lfariabr/review-pulse/issues/88) -> [#89](https://github.com/lfariabr/review-pulse/issues/89)<br>
...
- preserve the verified #84 four-model artifacts and evaluation evidence generated from commit `bf36c3b`;
- preserve the #85 exact-offset ATAE heatmap, DistilBERT gradient x input attribution and explicit unsupported baseline states;
...
Issue #84 closed on 24 July 2026 with 1,120 shared official-test predictions, 228 mixed-polarity examples, four verified model artifacts, efficiency records, confusion matrices and reviewed bidirectional error cases. DistilBERT led the predictive comparison; ATAE-LSTM improved mixed-subset macro-F1 over both review-only baselines but produced lower full-test accuracy, an unexpected result retained for #88.

Issue #85 closed via PR #97 on 25 July 2026. The v3 interface now presents exact-offset ATAE-LSTM attention and deterministic DistilBERT gradient x input attribution, explicitly marks review-only baselines as unsupported, exports the submitted `food`/`service` RQ3 example and limits every view to indicative rather than causal evidence. The merged candidate records 243 passing tests and 8 expected skips.
...
- the post-PR #97 baseline records 243 passing and 8 skipped tests; this suite must be rerun after any activated six-model work and packaging changes.
...
| [#84](https://github.com/lfariabr/review-pulse/issues/84) | Four-model comparison and error analysis | Juan + Group | Closed 24 Jul | Verified artifacts and one shared prediction file publish metrics, efficiency, matrices and bidirectional disagreements |
| [#85](https://github.com/lfariabr/review-pulse/issues/85) | Indicative token-evidence views | Luis | Closed via PR #97 | Exact-offset ATAE heatmap, DistilBERT attribution, unsupported baseline states and RQ3 export are verified |
...
environment + legacy baseline
  ...
  -> four-model runner [PR #93] -> verified experiment [#84, closed]
  -> token evidence [#85, closed via PR #97]
...
|---|---|---|---|
| Foundation + data + baseline | 18-26 Jul | Verified legacy suite, ABSA boundaries, audited Restaurants data, grouped splits and TF-IDF implementation | Code and verified #84 baseline evidence complete |
| LSTM + ATAE-LSTM | 27 Jul-2 Aug | Target-agnostic LSTM and ATAE-LSTM implementation, checkpoints and metrics | Code, #91 controls and verified #84 runs complete |
| DistilBERT | 3-8 Aug | Sentence-pair Transformer checkpoint and metrics | Code, deterministic run metadata and verified #84 run complete |
| Evaluation + interface | 9-13 Aug | Four-model comparison, error analysis, evidence views and Streamlit v3 | #84 and #85 complete |
| Gated six-model extension | After #85; maximum two implementation days | GRU, CNN and supplemental comparison | #94/#95 pass independently, then #96; cut without moving report/release dates |
...
| Remaining v3 changes regress the merged legacy/v3 test baseline | Medium | High | Preserve APIs and artifact paths; retain the 243-pass post-#85 baseline and rerun at extension/release gates | Revert the affected seam before experiments or packaging proceed |
...
1. Decide the extension activation gate. If the two-day budget remains, implement #94 and #95 independently; otherwise protect the completed four-model core.
2. Merge #94/#95 only after their isolated acceptance checks, then execute #96 to preserve the canonical four-model path and add the supplemental six-model path.
3. Freeze core and any accepted optional artifacts, then write #88 with four-model A2 results plus clearly labelled exploratory evidence.
4. Complete #89 only after clean-install CPU smoke, artifact/ZIP size checks and final source-to-report consistency.
