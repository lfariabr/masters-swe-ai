...
- **Activations vs gradients (the sharpest distinction in this module):** activations show *how the network decides*; gradients show *how the network learns*. Almost every DL visualisation is activation-based and post-training; RNNbow is the counterexample.
...
| **Difference distribution view** | Histogram + stacked chart, binned by **absolute** weight change, coloured by **relative percent difference** `d(x,y) = 2·|x−y| / (|x|+|y|) ∈ [0,2]` (used because ordinary relative change breaks when the denominator ≈ 0) | Are the changes big or small, and how are they distributed? (T2) |
