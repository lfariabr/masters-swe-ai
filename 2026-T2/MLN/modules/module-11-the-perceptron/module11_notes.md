...
> **One-line frame:** the **perceptron** (Rosenblatt, 1958) is the *first* artificial neural network and the
> atom every deep net is built from: **inputs × weights + bias → weighted sum → activation → output**. It learns
> by nudging weights after each misclassified example (`w ← w + η·(target − pred)·x`). It **only works on
> linearly separable data** and **converges** if such a separator exists (Perceptron Convergence Theorem). Its
...
> ⚠️ **Citation mismatch in two resources.** The local PDFs for Resources **2** and **6** are *not* the
> Sharma (2017) articles cited in `notes.md` — they are **Bhardwaj (2020)** "What is a Perceptron?" and
> **Lang (2024)** "Activation Functions in Neural Networks" respectively. Same topics, different authors/years.
> Summaries below reflect the **PDFs actually on file**; reconcile the citations (swap the reference, or
> re-download Sharma) before quoting them in an assessment.
...
**Citation (local PDF):** Bhardwaj, A. (2020, 11 October). *What is a perceptron? — Basics of neural networks*. Towards Data Science. *(notes.md cites Sharma, S. (2017), "What the hell is perceptron?" — see mismatch note above.)*
...
- **Binary classifier example:** with `wx=−0.5, wy=0.5, b=0`, the line `0.5x + 0.5y = 0` is the **decision
  boundary**; a **step** activation labels one side `1`, the other `0`.
...
its two fundamental limitations, and how **SVM and logistic regression** are its close cousins that fix them.
...
| **SVM** (Vapnik, ~1960s→90s) | output is an MP-unit ±1; **max-margin** boundary | handles **generalization** (margin) + **no-perfect-solution** (soft margin) |
| **Logistic regression** | **sigmoidal** MP-unit → probability 0-1 | smooth SVM; **probabilistic** generalization guarantee |
...
**Citation (local PDF):** Lang, N. (2024, 12 December). *Activation functions in neural networks: How to choose the right one*. Towards Data Science. *(notes.md cites Sharma, S. (2017), "Activation functions in neural networks" — see mismatch note above.)*
...
**guaranteed to converge on linearly separable data** (Perceptron Convergence Theorem) and **guaranteed to fail**
otherwise — the **XOR** problem that stalled the field until **multilayer networks + smooth, differentiable
activations** made **backpropagation** possible. It sits in a family with **SVM** (max-margin) and **logistic
regression** (sigmoid → probability), and its "memorization not generalization" guarantee is the perfect foil to
