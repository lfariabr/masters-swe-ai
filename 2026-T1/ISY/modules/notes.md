...
### TLDR
Intelligent systems are built on machine learning (ML), which has four core learning paradigms: **supervised** (task-driven, labelled data), **unsupervised** (data-driven, unlabelled), **semi-supervised** (hybrid), and **reinforcement** (environment-driven, reward/penalty). This module introduces these foundations alongside real-world applications (cybersecurity, healthcare, IoT, NLP) and the ethical considerations that must guide AI deployment — including fairness, bias, and differential privacy. ISY503 focuses primarily on supervised learning and models, with reinforcement learning for AI agents.

### Introduction
Intelligent systems have been studied in the following areas: speech recognition, computer vision and natural language processing. This subject focuses primarily on computer vision and natural language processing, as the field of speech recognition is nested in these two main areas. It also covers the basics of machine learning (ML), as intelligent systems are based on ML.

This module is an introduction to intelligent systems. It will provide you with an understanding of the reasoning behind intelligent systems and their applications. The essential resources included in this Module will help you to understand the complexities of implementing intelligent systems and the ethical principles that need to be considered in their application. This module will also help you prepare for the first assessment—a case study based on an intelligent system.

### Resources

#### 1. Overview of Intelligent Systems and Approaches
- Sarker, I. H. (2001). Machine learning: Algorithms, real-world applications and research directions. SN Computer Science, 2(3), 160. https://www.proquest.com/scholarly-journals/machine-learning-algorithms-real-world/docview/2933400158/se-2?accountid=176901

*Resource Overview:*

    Intelligent systems depend on the learning ability of computers and machines. There are four main machine learning approaches for intelligent systems: supervised learning, unsupervised learning, semi-supervised learning and reinforcement learning. You are probably familiar with reinforcement learning, as it is the basis for artificial intelligent (AI) agents and environment discovery in this subject. However, this subject mainly focuses on supervised learning and models. Please read the contents of the following webpage to further develop your understanding of the differences between these approaches and the machine learning models that each of them commonly use.

> *Status: ✅ Read + Reviewed — see [module01_notes.md](module_01_nature-and-purpose-of-intelligent-systems/module01_notes.md)*

#### 2. 10 Powerful Examples of AI Applications
- Magnimid. (2019, 15 March). 10 powerful examples of AI applications in today's world. Retrieved https://becominghuman.ai/10-powerful-examples-of-ai-applications-553f7f062d9f
...
*Resource Overview:*
    ...
    If you would like to learn more about the topics covered in this Module, some additional resources are listed below. These resources will provide you with a deeper understanding of the topics covered. However, these resources are not essential to the completion of this Module or the assessments associated with this subject.
...
A bank is using an intelligent system to grant loans. The bank gives each customer a form to complete. In the form, one question asks the customers to give reasons as to why they will not be late in repaying the loan instalments. The system uses computer vision to read the hand-writing of the customer. It also uses natural language processing to sense the feeling of their writing (e.g., if it is polite, friendly, formal, etc.). After a year in which 2,000 loan forms were collected, the bank trains a ML algorithm to grant loans only to those customers whose writing shares a similar feeling, as such customers are never late in their loan repayments.
...
- Post your answer to the Module 1 discussion forum.
...
- **Supervised learning** maps labelled input **features (X)** to output **targets (Y)** — classification produces a discrete class label; regression produces a continuous number.
- Data must be split into **training** (model learns from this) and **testing** (true generalisation measure) sets before fitting any model; mixing them causes optimistic bias (**data leakage**).
- Core evaluation metrics all derive from the **confusion matrix** (TP, TN, FP, FN): accuracy (balanced data), precision (FP-sensitive), recall (FN-sensitive), F1 (imbalanced data), and AUC (threshold-independent comparison).
- **Overfitting** — great training accuracy, poor test accuracy — is countered by **feature selection** (remove irrelevant/redundant features via filter/wrapper/embedded methods) and regularisation.
- **Categorical features** must be encoded numerically before most ML algorithms can process them; key strategies are **Label Encoding** (integers) and **One-Hot Encoding** (binary columns), with **target encoding** (regularised) best for high-cardinality features.
- For **reinforcement learning**, rewards replace labelled targets — the reward function quality determines learning quality; for **unsupervised learning** (clustering), internal metrics (Silhouette, Davies-Bouldin) evaluate structure quality without ground-truth labels.
...
> *Status: ✅ Read + Reviewed — see [module02_notes.md](./module_02_introduction-to-machine-learning/module02_notes.md)*
...
> *Status: 🔥 WIP — needs manual watch (see [module02_notes.md](./module_02_introduction-to-machine-learning/module02_notes.md) for summary)*
...
> *Status: 🔥 WIP — needs institutional library access (authenticated eBook via Torrens portal)*
...
- Zaccone, G. & Karim, M. (2018). Deep learning with Tensorflow: Explore neural networks and build intelligent systems with Python (2nd ed.). Birmingham, England: Packt. (pp. 31–73). Retrieved from https://ebookcentral-proquest-com.torrens.idm.oclc.org/lib/think/reader.action?docID=5340529
...
- **OpenCV** is the primary Python library for CV — ships pre-trained models and supports frame-level video processing
...
- **Object tracking** extends per-frame detection to video: frame differencing → colorspace thresholding → **CAMShift** → optical flow (**Lucas-Kanade**) → background subtraction
- **Transfer learning** reuses a pre-trained model's weights for a new task — replace the output layer, fine-tune, dramatically reduce data/compute needs
...
> *Status: ✅ Read + Reviewed — see [module08_notes.md](module_08_computer-vision/module08_notes.md)*
...
- CrashCourse. (2017, 22 November). Natural language processing: Crash course computer science #36 [Video file]. Retrieved from https://www.youtube.com/watch?v=fOvTtapxa9c

*Resource Overview:*

    Watch the following video, which provides an overview of NLP. Take note of the following concepts discussed:
    - Why NLP is important;
    - The history of NLP;
    - Sentence structures and its implication for NLP;
    - Applications of NLP;
    - Statistics about sequence of words;
...
> *Status: ✅ Read + Reviewed — see [module11_notes.md](module_11_emotional-intelligence/module11_notes.md)*
...
> *Status: 🔥 WIP — LMS-gated; summarised from course overview — see [module11_notes.md](module_11_emotional-intelligence/module11_notes.md)*
...
> *Status: ✅ Watched + Reviewed — see [module11_notes.md](module_11_emotional-intelligence/module11_notes.md)*
