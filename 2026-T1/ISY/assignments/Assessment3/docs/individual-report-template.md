# Individual Contribution Report Template - ISY503 Assessment 3

Target length: 250 words +/- 10%.

## Student Details

- Name:
- Student ID:
- Project:
- GitHub repository:

## Team Contribution Table

| Team Member | Student ID | Main Contribution | Percentage |
|---|---|---|---:|
| TBD | TBD | TBD | TBD |
| TBD | TBD | TBD | TBD |
| TBD | TBD | TBD | TBD |
| TBD | TBD | TBD | TBD |
| **Total** |  |  | **100%** |

## Draft Report

I contributed to the NLP sentiment analysis project by [describe your main technical ownership: parser, preprocessing, model training, evaluation, Streamlit app, documentation, presentation]. My work focused on [specific implementation details] and helped the team meet the assessment requirements by [connect to brief: trained neural network, inference UI, evaluation, ethics, presentation].

The team divided responsibilities as follows: [summarise each member's contribution and percentage]. I estimate my contribution at [X%] because [brief justification based on code, documentation, meetings, or presentation work].

An important ethical consideration in this project is that sentiment labels can contain noise and ambiguity. Although the dataset is split into positive and negative review files, star ratings and review text may not always align. Some reviews may be mixed, sarcastic, or domain-specific, and binary classification can oversimplify real user sentiment. We addressed this by auditing labels, documenting ambiguous cases, and reporting model limitations rather than presenting the system as universally accurate.

## APA References To Include

- Dataset source: Blitzer, M. Dredze, and Pereira (2007)
- NLP/sentiment method reference
- AI ethics or bias reference
