...
| PCA | Dimensionality reduction — rotates + compresses features into orthogonal components. Not normalization. |
| Gradient clipping | Caps gradient magnitude before weight update — prevents NaN without needing label normalization |
| Learning rate decay | Reduces lr over training steps — start aggressive, refine as loss converges |
| Regularization (L1/L2) | Penalises large weights in the loss function to prevent overfitting |
| Dropout | Randomly deactivates neurons during training — strong regulariser for DNNs |
