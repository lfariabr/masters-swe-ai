...
Expected test-set RMSE: Task 1 RF ~$2,873; Task 2 RF+Z-score ~$2,875; Task 3 MLP+One-Hot ~$2,812; Task 4 RF ~$2,777.
...
*Figure 1. Experimental progression across Tasks 1–4. Task 2's normalisation degradation, reproduced with Adam at lr=1e-3, mirrors the pattern from the TF1 pipeline. T4 RF ($2,777) and T3 MLP+One-Hot ($2,812) are the top two results, separated by $35.*

The notebook evaluated a structured set of baselines and alternatives across tasks, including `LinearRegression`, Keras MLP variants with hidden layers [64], [64,32], [128,64], Wide & Deep, and `RandomForestRegressor` (500 trees). Not every task used the same number of configurations, because the model set was adapted to the feature-engineering goal of that task. Task 3 switched MLP to Adagrad (lr=0.1) to compensate for reduced gradient magnitudes with sparse one-hot inputs (Duchi et al., 2011). Random forests were the most consistent architecture — scale-invariant, no learning rate to tune, and best or near-best across all tasks; their ensemble averaging suits the 120-row training set better than deep parameterisation (Breiman, 2001).
...
*Figure 2. Test-set RMSE by model variant (lower = better). RF variants cluster between $2,777–$3,913 across all tasks. T4 RF ($2,777) is the best model; T2 MLP+Z-score ($14,109) is the worst, suggesting the degradation is not specific to the earlier TF1 setup.*
...
*Figure 3. Normalised efficiency score — higher bars = better. Score = 100 × (worst\_RMSE − model\_RMSE) / (worst\_RMSE − best\_RMSE), where worst = T2 MLP+Z-score ($14,109) and best = T4 RF ($2,777). T4 RF and T3 MLP+One-Hot both score near 100 — they are $35 apart; RF variants score consistently high throughout.*

T4 RF ($2,777) produced the lowest test RMSE this run, marginally ahead of T3 MLP+One-Hot ($2,812) — a $35 gap on a $13,210 mean price. Both are plausible best-in-class results; RF has the additional advantage that it reproduces identically across runs, while MLP results varied. For MLP specifically, T3 still outperforms T4: adding Z-score numerics to one-hot inputs appears to increase overfitting pressure on 120 training rows, whereas RF selects relevant features per split and is not penalised by the extra dimensions (Sarker, 2021).

Normalisation again degraded MLP performance (RMSE $14,109 vs. $5,299 un-normalised). A likely explanation is the same pattern seen in the TF1 pipeline: Z-score compresses inputs to ~±3, reducing gradient magnitudes, and Adam at lr=1e-3 may then update too slowly for the model to learn useful variation, leaving predictions close to the dataset mean ($13,210) instead (LeCun et al., 2015). The RF control confirmed scale-invariance: RF+Z-score ($2,875) ≈ RF+None ($2,873), a two-dollar difference.
...
### Appendix C — RandomForest Cross-Task Consistency
