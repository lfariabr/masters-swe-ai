...
#### Forum Discussion:
PwC built the Cyber Technology Rationalizer to help its clients — CIOs and CTOs — forecast cybersecurity costs and model coverage scenarios. The legacy system (React, Node.js, PostgreSQL) was the core challenge: business logic was embedded in stored procedures, making every change dependent on specialist developers. Clients also lacked self-service capability, relying entirely on PwC consultants to run scenarios on their behalf — a bottleneck that limited scalability and responsiveness.

The service model is primarily **Platform as a Service (PaaS)**. Microsoft manages the underlying compute, runtime, and database infrastructure; PwC's team focuses solely on application logic and user experience. Microsoft 365 integrations (SharePoint, collaboration) sit at the SaaS layer. There is no IaaS footprint — no raw virtual machines or unmanaged storage were provisioned.

The Microsoft Power Platform services used are: **Power Pages** (multitenant web portal), **Microsoft Dataverse** (managed data layer with Web API), **C# plugins** (business logic computation), **Power Apps component framework** (React component integration), and **Microsoft Azure** (underlying security, compliance, and data governance).

Each service plays a distinct architectural role. Power Pages hosts the portal and enforces tenant isolation — each client accesses only their own data. When a user submits their cyber licensing portfolio, C# plugins in Dataverse execute the business logic and return JSON results via the Power Pages Web API, which the frontend transforms into interactive cost and coverage visuals. The Power Apps component framework allowed PwC to reuse existing React components directly, avoiding a full frontend rewrite and cutting build time significantly. The result: the Cyber Technology Rationalizer was delivered in six weeks, achieving 85% cost savings and 30% time savings compared to the legacy system — with reusable components already accelerating future Power Platform projects.